...
class DbBookmark(DbBase):
    ...
    SQL_BOOKMARK_PREVIOUS   = """SELECT * FROM BookmarkView WHERE book_id = ? and page <  ? ORDER BY page DESC LIMIT 1"""
    ...
    def _getBookID(self, book:str|int )->int:
        if self._last_book_value != book:
            self._last_book_value = book
    ...
    def getPreviousBookmarkForPage( self, book:str|int, page:int )->dict:
        ...
        id = self._getBookID( book )
        res = DbHelper.fetchrow( DbBookmark.SQL_BOOKMARK_PREVIOUS, [id, page] , DbBookmark.joinView, debug=False )
        return res
        
    def getNextBookmarkForPage( self, book:str|int, page:int )->dict:
        id = self._getBookID(book)
        res = DbHelper.fetchrow( DbBookmark.SQL_BOOKMARK_NEXT, [id, page] , DbBookmark.joinView)
        return res
    ...
    def delete(self, bookmark:str, book:str|int=None  )->bool:
        ...
        query = DbHelper.bind( query , [ self._getBookID( book ), bookmark] )
        query.exec()
        self._checkError( query )
    ...
    def delete_by_page(self, book:str|int, page:int ):
        ...
        if row is not None and len( row ) > 0 and BOOKMARK.name in row:
            return self.delete( bookmark=row[BOOKMARK.name] , book_id=row[BOOKMARK.book_id] )
        return False
    ...
    def getAll(self, book:str|int, order:str='page' )->list:
        ...
        return self.getAllId( book , order=order )
    ...
    def getAllId(self,book:str|int, order:str='page')->list:
        ...
        q = DbHelper.prep(sql)
        q = DbHelper.bind( q , self._getBookID( book ) )
        q.exec()
